...
const ai = genkit({
  plugins: [
    ...
    // by passing in a config object; if you don't, the Vertex AI plugin
    // uses the value from the GCLOUD_PROJECT environment variable.
...
const menuSuggestionFlow = ai.defineFlow({
  ...
  // Construct a request and send it to the model API. Genkit models
  // generally depend on an API key. APIs should be stored in Cloud Secret
  // Manager so that access to these sensitive values can be controlled.
...
export const menuSuggestion = onCallGenkit({
  ...
  // your Verified app users can use your API. Read more at
