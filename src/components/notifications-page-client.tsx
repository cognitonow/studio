...
interface NotificationListProps {
  ...
  role: 'client' | 'provider' | 'guest';
}

const NotificationList = ({ items, onItemClick, role }: NotificationListProps) => {
    ...
    <div className="space-y-4">
      {items.length > 0 ? (
        items.map((notification) => {
          ...
          const wrapperProps = isClickable ? { href: getNotificationLink(notification), onClick: () => onItemClick(notification.id) } : {};
          ...
          return (
            <Wrapper key={notification.id} {...wrapperProps} className="block">
              ...
              >
                ...
                {!notification.read && <div className="w-2.5 h-2.5 rounded-full bg-primary shrink-0 mt-2"></div>}
              </div>
            </Wrapper>
          );
        })
...
interface NotificationsPageClientProps {
  initialClientNotifications: Notification[];
  initialProviderNotifications: Notification[];
}
...
export function NotificationsPageClient({
  ...
  useEffect(() => {
    // Determine which list to display based on the current role.
    const currentNotifications = role === 'provider' ? initialProviderNotifications : initialClientNotifications;
    setNotifications(currentNotifications);

    // This function will re-fetch from the mock data source to keep UI fresh
    const refreshData = () => {
        if (role !== 'guest') {
  ...
  }, [role, initialClientNotifications, initialProviderNotifications]);

  const handleMarkAllAsRead = () => {
    if (role !== 'guest') {
      markAllNotificationsAsRead(role);
      setNotifications(getNotifications(role)); // Re-fetch to update UI
    }
  };
    ...
    <div className="container mx-auto py-12 px-4">
      <div className="max-w-3xl mx-auto">
        <Card>
          ...
          <CardContent>
            <NotificationList items={notifications} onItemClick={handleItemClick} role={role} />
          </CardContent>
        </Card>
      </div>
    </div>
