...
function ExploreStack({ providers }: ExploreStackProps) {
  ...
  if (!providers || providers.length === 0) {
    return <p>No providers available to explore.</p>;
  }
...
interface DiscoverPageClientProps {
    ...
    allServicesGrouped: (ServiceCategory & { services: Service[] })[];
    dublinDistricts: DublinDistrict[];
    initialPlaylists: Record<string, Provider[]>;
}
...
export function DiscoverPageClient({
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    if(newCategory && categoryToPlaylistMap[newCategory]) {
        setPlaylistProviders(initialPlaylists[categoryToPlaylistMap[newCategory]] || []);
    } else {
        setPlaylistProviders(featuredProviders);
    }
  }

  const filteredServices = selectedCategory ? allServicesGrouped.find(g => g.id === selectedCategory)?.services ?? [] : [];
      ...
      <section>
        ...
        <Tabs defaultValue="explore">
          ...
          <TabsContent value="explore">
            <ExploreStack providers={allProviders} />
          </TabsContent>
