...
interface MessagesPageClientProps {
  initialClientConversations: Conversation[];
  initialProviderConversations: Conversation[];
  initialClientMessages: Message[];
  initialProviderMessages: Message[];
  initialActiveClientConvo?: Conversation;
  initialActiveProviderConvo?: Conversation;
}

export function MessagesPageClient({
  initialClientConversations,
  initialProviderConversations,
  initialClientMessages,
  initialProviderMessages,
  initialActiveClientConvo,
  initialActiveProviderConvo,
  ...
  const fetchClientConversations = () => getConversations();
  const fetchProviderConversations = () => getProviderConversations();
  
  const handleConversationSelect = (convo: Conversation) => {
    setActiveConversation(convo);
    const newMessages = userRole === 'provider' 
        ? getProviderMessagesForConversation(convo.id)
        : getMessagesForConversation(convo.id);
    setActiveMessages(newMessages);

    if (userRole !== 'guest' && convo.unread > 0) {
      markAllMessagesAsRead(convo.id, userRole);
      // Re-fetch conversations to update the unread count in the UI
  ...
  useEffect(() => {
    ...
    let initialActiveConvo: Conversation | undefined;
    let initialMessages: Message[] = [];
    const providerIdToChat = searchParams.get('providerId');

    if (userRole === 'provider') {
        convos = initialProviderConversations;
        initialActiveConvo = initialActiveProviderConvo;
        initialMessages = initialProviderMessages;
    } else { // client
        convos = initialClientConversations;
        initialActiveConvo = initialActiveClientConvo;
        initialMessages = initialClientMessages;
    }
    
    setConversations(convos);

    // This logic handles opening a specific chat via URL param
    if (userRole === 'client' && providerIdToChat) {
        let convoToSelect = convos.find(c => c.providerId === providerIdToChat);
        if (!convoToSelect) {
        ...
        if (convoToSelect) {
            handleConversationSelect(convoToSelect);
        }
        router.replace('/messages', {scroll: false}); // Clean up URL
    } else {
        setActiveConversation(initialActiveConvo);
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setConversations(userRole === 'provider' ? fetchProviderConversations() : fetchClientConversations());
    setActiveMessages(userRole === 'provider' ? getProviderMessagesForConversation(activeConversation.id) : getMessagesForConversation(activeConversation.id));
  }
