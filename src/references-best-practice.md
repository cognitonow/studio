...
- **Controlled Components for Dialogs:** When a dialog needs to manage its own internal state (e.g., form inputs) *and* interact with a parent component's state, it must be implemented as a "controlled component."
    - The parent component owns the state for the dialog's visibility (`isOpen`).
    - The parent component passes a callback function to the dialog to receive data.
    - The dialog's trigger (`<DialogTrigger>`) resides in the parent component.
